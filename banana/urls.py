from django.conf.urls import patterns, url
from banana.views import images, lists, details
from banana.views.etc import extracted_sources_pixel


urlpatterns = patterns('',
    url(r'^$',
        lists.databases,
        name='databases'),
    url(r'^(?P<db_name>\w+)/$',
        lists.datasets,
        name='datasets'),
    url(r'^(?P<db_name>\w+)/images/$',
        lists.images,
        name='images'),
    url(r'^(?P<db_name>\w+)/transients/$',
        lists.transients,
        name='transients'),
    url(r'^(?P<db_name>\w+)/extractedsources/$',
        lists.extractedsources,
        name='extractedsources'),
    url(r'^(?P<db_name>\w+)/runningcatalogs/$',
        lists.runningcatalogs,
        name='runningcatalogs'),
    url(r'^(?P<db_name>\w+)/monitoringlists/$',
        lists.monitoringlists,
        name='monitoringlists'),

    url(r'^(?P<db_name>\w+)/transient/(?P<pk>\d+)/$',
        details.TransientDetail.as_view(),
        name='transient'),
    url(r'^(?P<db_name>\w+)/dataset/(?P<pk>\d+)/$',
        details.DatasetDetail.as_view(),
        name='dataset'),
    url(r'^(?P<db_name>\w+)/extractedsource/(?P<pk>\d+)/$',
        details.ExtractedSourceDetail.as_view(),
        name='extractedsource'),
    url(r'^(?P<db_name>\w+)/runningcatalog/(?P<pk>\d+)/$',
        details.RunningcatalogDetail.as_view(),
        name='runningcatalog'),
    url(r'^(?P<db_name>\w+)/monitoringlist/(?P<pk>\d+)/$',
        details.MonitoringlistDetail.as_view(),
        name='monitoringlist'),
    url(r'^(?P<db_name>\w+)/image/(?P<pk>\d+)/$',
        details.ImageDetail.as_view(),
        name='image'),
    url(r'^(?P<db_name>\w+)/bigimage/(?P<pk>\d+)/$',
        details.BigImageDetail.as_view(),
        name='bigimage'),
    url(r'^(?P<db_name>\w+)/extractedsourcespx/(?P<image_id>\d+)/$',
        extracted_sources_pixel,
        name='extracted_sources_pixel'),

    url(r'^(?P<db_name>\w+)/extractedsourceplot/(?P<extractedsource_id>\d+)/$',
        images.extractedsource_plot,
        name='extractedsource_plot'),
    url(r'^(?P<db_name>\w+)/imageplot/(?P<image_id>\d+)/$',
        images.image_plot,
        name='image_plot'),
    url(r'^(?P<db_name>\w+)/transientplot/(?P<transient_id>\d+)/$',
        images.transient_plot,
        name='transient_plot'),
    url(r'^(?P<db_name>\w+)/lightcurveplot/(?P<runningcatalog_id>\d+)/$',
        images.lightcurve_plot,
        name='lightcurve_plot'),
    url(r'^(?P<db_name>\w+)/scatterplot/(?P<dataset_id>\d+)/$',
        images.scatter_plot,
        name='scatter_plot'),
)
